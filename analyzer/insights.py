...
class Report:
    ...
    def feature_segment(self):
        ...
        num_col = df.select_dtypes(include=['int64', 'int32', 'float64', 'float32'])
        cat_cols = df.select_dtypes(include=['object'])
        datetime_cols = df.select_dtypes(include=['datetime64[ns]'])

        return num_col, cat_cols, datetime_cols
    ...
    def basic_data_details(self):
        ...
        num_col_shape = len(num_col.columns)
        cat_col_shape = len(cat_col.columns)
        datetime_col_shape = len(datetime_col.columns)
        return {'data_shape': df.shape, 'num_col_shape':num_col_shape, 'cat_col_shape':cat_col_shape, 'datetime_col_shape': datetime_col_shape,'null_counts':null_counts }
    ...
    def get_unibarChart(self):
        ...
        col_size = df.shape[1]
        cat_cols = df.select_dtypes(include=['object'])
        limited_cat_cols = [col for col in cat_cols.columns if (df[col].nunique() < 15) and (df[col].nunique() >= 2) ]
    ...
    def get_unihistChart(self):
        ...
        num_col, cat_col, datetime_col = self.feature_segment()
        voilin_plot_col = [col for col in num_col if (df[col].std() > 5) and (max(df[col].value_counts().to_dict().values()) / df.shape[0] > 0.5 )]
        hist_plot_col = [col for col in num_col.columns if (df[col].std() > 5) and (col not in voilin_plot_col)]
        hist_plots = []
    ...
    def get_biboxplot(self):
        ...
        voilin_plot_col = [col for col in num_col if (df[col].std() > 5) and (max(df[col].value_counts().to_dict().values()) / df.shape[0] > 0.5 )]
        hist_plot_col = [col for col in num_col.columns if (df[col].std() > 5) and (col not in voilin_plot_col)]
        limited_cat_cols = [col for col in cat_col.columns if (df[col].nunique() < 15) and (df[col].nunique() > 2) ]
        
        box_plots = []
        # x_axis = limited_cat_cols
        # y_axis = hist_plot_col, if hist_plot_col <= 2 then y_axis = num_col
        print(len(hist_plot_col))
        if len(hist_plot_col) <=2 :
            y_axis_col = num_col.columns
            print(y_axis_col)
        else:
    ...
    def get_scatterplot(self):
        ...
        scatter_plt = list(combinations([feature for feature in num_col.columns if df[feature].nunique()/df.shape[0] >= 0.4], 2))
        scatter_plots = []
